...
<h1><?php _e('Manager Fonts', $this->textdomain); ?></h1>
<h2><?php echo __('Custom Fonts', $this->textdomain); ?></h2>
<?php echo $notice; ?>
<div class="wrap nbdesigner-container">
	<div class="nbdesigner-content-full">
		<form name="post" action="" method="post" enctype="multipart/form-data" autocomplete="off">
			<div class="nbdesigner-content-left postbox">
				<h3><?php echo __('Add font', $this->textdomain); ?></h3>
				<div class="inside nbdesigner_font_info">						
					<?php wp_nonce_field($this->plugin_id, $this->plugin_id.'_hidden'); ?>		
					<table class="form-table">
						<tr valign="top">
							<th scope="row" class="titledesc"><?php echo __("Font name", $this->textdomain); ?> </th>
						...
						<tr valign="top">
							<th scope="row" class="titledesc"><?php echo __("Font file", $this->textdomain); ?> </th>
							<td class="forminp-text">
								<input type="file" name="woff" value="" /><br />
								<div style="font-size: 11px; font-style: italic;"><?php _e('Allow extensions: woff, ttf', $this->textdomain); ?></div>
							</td>
						</tr>				
					</table>
					<input type="hidden" name="nbdesigner_font_id" value="<?php echo $font_id; ?>"/>
					<p class="submit">
						<input type="submit" name="Submit" class="button-primary" value="<?php _e('Save', $this->textdomain); ?>" />
						<a href="?page=nbdesigner_manager_fonts" class="button-primary" style="<?php $style = (isset($_GET['id'])) ? '' : 'display:none;';echo $style; ?>"><?php _e('Add New', $this->textdomain); ?></a>
					</p>									
				</div>	
				<?php if(isset($font_data)): ?>
				<div class="nbdesigner_font_preview">
					<h4><?php _e('Preview', $this->textdomain); ?></h4>
					<div>
						<style type='text/css'>
			...
			<div class="nbdesigner-content-side">
				<div class="postbox">
					<h3><?php _e('Categories', $this->textdomain); ?><img src="<?php echo NBDESIGNER_PLUGIN_URL . 'assets/images/loading.gif'; ?>" class="nbdesigner_editcat_loading nbdesigner_loaded" style="margin-left: 15px;"/></h3>
					<div class="inside">
						...
						<?php if(is_array($cat) && (sizeof($cat) > 0)): ?>							
							...
							<?php endforeach; ?>							
						<?php else: ?> 
							<li><?php _e('You don\'t have any category.', $this->textdomain); ?></li>
						<?php endif; ?>
						</ul>
						<input type="hidden" id="nbdesigner_current_font_cat_id" value="<?php echo $current_font_cat_id; ?>"/>
						<p><a id="nbdesigner_add_font_cat"><?php _e('+ Add new font category', $this->textdomain); ?></a></p>
	...
	<div class="clear"></div>
	<div class="postbox">
		<h3><?php echo __('List fonts: ', $this->textdomain).$name_current_cat; ?><a class="nbdesigner-right" href="<?php echo admin_url('admin.php?page=nbdesigner_manager_fonts'); ?>"><?php _e('All fonts', $this->textdomain); ?></a></h3>
		<div class="nbdesigner-list-fonts inside">				
			<div class="nbdesigner-list-fonts-container">
				<?php if(is_array($list) && (sizeof($list) > 0)): ?>
					...
					<?php endforeach; ?>
				<?php else: ?>
					<?php _e('You don\'t have any custom font.', $this->textdomain);?>
				<?php  endif; ?>
			</div>
		</div>
	</div>
</div>
<h2><?php echo __('Google Fonts', $this->textdomain); ?></h2>
<div class="wrap nbdesigner-container">
	<div class="postbox">
		<div class="inside nbdesigner_google_added">
			<div class="ui-widget"><input type="search" id="nbdesigner_google_font_seach"/><input onclick="NBDESIGNADMIN.add_google_font(this)" type="button" value="<?php _e('Add', $this->textdomain); ?>" class="button"/><img src="<?php echo NBDESIGNER_PLUGIN_URL.'assets/images/loading.gif' ?>" class="nbdesigner_loaded" id="nbdesigner_google_font_loading" style="margin-left: 15px;"/></div>		
			...
			<?php if(is_array($data_font_google)): ?>
				...
				<?php endforeach; ?>
			<?php else: ?>
				<p id="nbdesigner_no_google_font"><?php _e('You don\'t have any google font', $this->textdomain); ?></p>
			<?php endif; ?>
